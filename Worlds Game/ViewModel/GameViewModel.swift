...
class GameViewModel: ObservableObject {
    ...
    func validate(word: String) throws {
        ...
        guard word != self.word else {
            ...
            throw WordError.theSamWord
        }
        guard !(words.contains(word)) else {
            print("Придумай новое слово которое не было составленно раннее")
            throw WordError.beforeWord
        }
        
        guard word.count > 1 else {
            print("Слишком короткое слово")
            throw WordError.littlWord
        }
        
        return
    }
    
    func wordToChart(word: String) ->[Character] {
    ...
    func  check(word: String) throws -> Int {
        ...
        var bigWordArray = wordToChart(word: self.word)
        let smallWordArray = wordToChart(word: word)
        var result = ""
